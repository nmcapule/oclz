...
class LazadaClient:
  ...
  def __init__(self, domain, useremail, api_key):
    self._domain = domain
  ...
  def _Request(self, action, payload='', offset=None, limit=None, body_xml_parser=None):
    ...
    parameters['Signature'] = HMAC(self._api_key, concatenated, sha256).hexdigest()
    concatenated = urllib.urlencode(sorted(parameters.items()))
    ...
    if e.tag == 'ErrorResponse':
      ...
      raise UnhandledTagError('Unknown tag found in response: %s' % e.tag)
  ...
  def Refresh(self):
    ...
    def xml_parser(body_xml_etree):
      outer_scope['total'] = int(body_xml_etree.find('TotalProducts').text)
      ...
      for product in body_xml_etree.find('Products').findall('Product'):
        ...
        reserved = quantity - int(sku.find('Available').text or quantity)
  ...
  def UpdateProducts(self, products):
    ...
    for p in products:
      ...
      result = self._Request(_UPDATE_PRODUCT_QUANTITY_ACTION, payload=payload)
      result.attachment = p
